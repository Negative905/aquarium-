...
const REEF_IMAGES = [
  ...
  { src: "/reef3.png", alt: "SPS Coral Display" },
  { src: "/reef4.png", alt: "LPS Reef Aquarium" },
];

const SOURCE = "reef-aquariums";
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          `${API}/products?categories=69b23d44da5543e64646dce7`,
        );
        ...
        const mapped = data.map((item, index) => ({
          ...
          image: REEF_IMAGES[index]?.src || "/reef1.png",
          alt: REEF_IMAGES[index]?.alt || item.name,
        }));
        setProducts(mapped);
      } catch (err) {
        console.error("Failed to fetch reef products:", err);
      } finally {
        setLoading(false);
      }
    };
