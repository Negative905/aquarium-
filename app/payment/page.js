...
import { getCart, saveCart } from "../../components/Cart";
...
export default function PaymentPage() {
  ...
  const handlePay = async () => {
    ...
    if (!orderData) {
      setError("Order data missing. Please go back to cart.");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${API}/orders`, {
        ...
        body: JSON.stringify({
          orderItems,
          shippingAddress1: orderData.shippingAddress1 || "N/A",
          shippingAddress2: "",
          city: orderData.city || "N/A",
          zip: orderData.zip || "000000",
          country: orderData.country || "India",
          phone: orderData.phone || "0000000000",
          status: "Pending",
          // ✅ Only send user if it's a real MongoDB ID
      ...
      // ✅ Clear cart and orderSummary after successful order
      saveCart([]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-12 bg-[linear-gradient(180.8deg,#065EB6_0.68%,#000C18_99.32%)]">
      <div className="w-full max-w-lg">
        ...
        <div className="flex gap-3 mb-5">
          ...
          <button className="flex-1 flex items-center justify-center py-2.5 px-4 rounded-md border border-white/30 bg-white/10 hover:bg-white/20 transition">
            <span className="flex items-center gap-1.5">
              ...
              >
                <path
                  d="M46 24.0287C46 22.09 45.8533 20.68 45.5013 19.2112H23.4694V27.9356H36.4069C36.1429 30.1094 34.7347 33.37 31.5957 35.5731L31.5671 35.7669L38.5703 41.0508L39.0544 41.0979C43.5258 36.9976 46 31.0584 46 24.0287Z"
                  fill="#4285F4"
                />
                <path
                  d="M23.4694 47C29.8061 47 35.1071 44.9991 39.0544 41.0979L31.5957 35.5731C29.6551 36.9669 27.0998 37.9306 23.4694 37.9306C17.268 37.9306 12.0276 33.8303 10.1918 28.2109L10.0114 28.2268L2.72046 33.7175L2.62109 33.8927C6.54271 41.7677 14.4322 47 23.4694 47Z"
                  fill="#34A853"
                />
                <path
                  d="M10.1918 28.2109C9.71286 26.7421 9.43681 25.1762 9.43681 23.5C9.43681 21.8238 9.71286 20.2579 10.1918 18.7891L10.1783 18.5745L2.79888 12.9979L2.62109 13.1073C1.07678 16.1822 0.187378 19.7412 0.187378 23.5C0.187378 27.2588 1.07678 30.8178 2.62109 33.8927L10.1918 28.2109Z"
                  fill="#FBBC05"
                />
                <path
                  d="M23.4694 9.07688C27.8699 9.07688 30.8622 10.9863 32.5344 12.5725L39.1934 6.11C35.0815 2.33917 29.8061 0 23.4694 0C14.4322 0 6.54271 5.23233 2.62109 13.1073L10.1918 18.7891C12.0276 13.1697 17.268 9.07688 23.4694 9.07688Z"
                  fill="#EA4335"
                />
              </svg>
        ...
        {/* Error message */}
        {error && (
          <div className="mb-4 px-4 py-3 bg-red-500/20 border border-red-400/30 rounded text-red-300 text-sm">
            {error}
          </div>
        )}
        ...
        <button
          onClick={handlePay}
          disabled={loading}
          className="w-full py-3 rounded-md bg-white text-black font-bold text-sm hover:bg-white/90 transition disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {loading ? "Placing Order..." : `Pay ${fmt(finalTotal)}`}
        </button>
      </div>
    </div>
  );
