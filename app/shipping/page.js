...
export default function ShippingPage() {
  ...
  // Order summary state pulled from cart
    ...
    <div className="min-h-screen flex items-center justify-center py-12 px-6 bg-[linear-gradient(180.8deg,#065EB6_0.68%,#000C18_99.32%)]">
      <div className="flex gap-6 w-full max-w-4xl">
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1 flex flex-col gap-1">
              ...
              <div className="flex rounded-md overflow-hidden border border-white/20 focus-within:border-white/60 transition">
                ...
                <input type="tel" placeholder="+91 9876567898"
          ...
          <div className="flex gap-4">
            <button onClick={() => setShippingMethod("free")}
        ...
        <div className="w-80 shrink-0 flex items-start pt-24 pl-6">
          <div className="w-full p-7 border border-white/20"
            ...
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="text-white text-sm font-semibold">{fmt(summary.subtotal)}</span>
            </div>
            <div className="flex justify-between items-center mb-3">
              <span className="text-white/80 text-sm">Discount (20%)</span>
              <span className="text-green-400 text-sm font-semibold">-{fmt(summary.discount)}</span>
            </div>
            {summary.promoDiscount > 0 && (
              <div className="flex justify-between items-center mb-3">
                <span className="text-white/80 text-sm">Promo ({summary.promoCode})</span>
                <span className="text-green-400 text-sm font-semibold">-{fmt(summary.promoDiscount)}</span>
              </div>
            )}
            <div className="flex justify-between items-center mb-5">
              <span className="text-white/80 text-sm">Delivery Fee</span>
              <span className="text-white text-sm font-semibold">{fmt(shippingCost)}</span>
            </div>
            ...
            <div className="flex justify-between items-center mb-5">
              ...
              <span className="text-white font-bold text-sm">{fmt(adjustedTotal)}</span>
            </div>

            {/* Promo display (read-only, already applied from cart) */}
            {promoCode ? (
