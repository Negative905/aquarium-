...
const SALTWATER_IMAGES = [
  ...
  { src: "/saltwater3.png", alt: "Fish-Only Marine Setup" },
  { src: "/saltwater4.png", alt: "Large Oceanic Tank" },
];
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          `${API}/products?categories=69b23d3bda5543e64646dce5`,
        );
        ...
        const mapped = data.map((item, index) => ({
          ...
          image: SALTWATER_IMAGES[index]?.src || "/saltwater1.png",
          alt: SALTWATER_IMAGES[index]?.alt || item.name,
        }));
        setProducts(mapped);
      } catch (err) {
        console.error("Failed to fetch saltwater products:", err);
      } finally {
        setLoading(false);
      }
    };
