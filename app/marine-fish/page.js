...
const LOCAL_IMAGES = {
  ...
  "Oscar Fish": "/fish5.png",
  "Molly Fish": "/fish6.png",
  "Neon Tetra": "/fish7.png",
  Cichlid: "/fish8.png",
  "Clown Fish": "/fish5.png",
  "Blue Tang": "/fish6.png",
  "Butterfly Fish": "/fish7.png",
  "Royal Gramma": "/fish8.png",
  Seahorse: "/fish9.png",
  Starfish: "/fish10.png",
...
export default function MarineFishPage() {
    ...
    const fetchProducts = async () => {
      try {
        ...
        const res = await fetch(
          `${API}/products?categories=69b2500eda5543e64646dd30`,
        );
        const data = await res.json();
        const mapped = data.map((p) => ({
          ...p,
          id: p._id,
          numericPrice: Number(p.price), // ✅ force number for cart
          image: LOCAL_IMAGES[p.name] || "/fish1.png",
        }));
        setProducts(mapped);
      } catch (err) {
        console.error("Failed to fetch marine fish:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleAddToCart = (product) => {
    addToCart({ ...product, source: "marine-fish", quantity: 1 });
    setQuantities((prev) => ({ ...prev, [product.id]: 1 }));
  };
  const handleIncrease = (product) => {
    const newQty = (quantities[product.id] || 0) + 1;
    setQuantities((prev) => ({ ...prev, [product.id]: newQty }));
    addToCart({ ...product, source: "marine-fish", quantity: newQty });
  };
