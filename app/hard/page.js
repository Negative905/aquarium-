...
const HARD_CORAL_IMAGES = [
  { src: "/coral5.png", alt: "Acropora" },
  { src: "/coral6.png", alt: "Brain Coral" },
  { src: "/coral7.png", alt: "Hammer Coral" },
  { src: "/coral8.png", alt: "Torch Coral" },
];
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          `${API}/products?categories=69b2369cda5543e64646dcb8`,
        );
        ...
        const mapped = data.map((item, index) => ({
          ...
          image: HARD_CORAL_IMAGES[index]?.src || "/coral5.png",
          alt: HARD_CORAL_IMAGES[index]?.alt || item.name,
        }));
        setProducts(mapped);
      } catch (err) {
        console.error("Failed to fetch hard corals:", err);
      } finally {
        setLoading(false);
      }
    };
