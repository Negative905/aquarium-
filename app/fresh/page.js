...
export default function FreshwaterAquariumsPage() {
  ...
  const handleDecrease = (product) => {
    ...
    if (current <= 1) {
      ...
      setQuantities(prev => { const u = { ...prev }; delete u[product.id]; return u })
    } else {
      updateQty(product.id, SOURCE, -1)
      ...
      <div className="pt-16">
        ...
        <section className="px-4 sm:px-8 lg:px-16 pb-20">
          <div className="max-w-6xl mx-auto">
            ...
            {/* Grid:
                - mobile: 1 column, fixed-height image (no giant gap)
                - sm+: original 2-column layout with tall portrait ratio, unchanged
            */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-8 sm:gap-y-24">
              {sortedProducts.map((product) => (
                <div key={product.id} className="rounded-sm bg-[#0a1e38] overflow-hidden shadow-md group">
                  ...
                  {/* Image wrapper:
                      mobile  → fixed h-56 so it doesn't eat the whole screen
                      sm+     → original padding-bottom 105% trick (tall portrait)
                  */}
