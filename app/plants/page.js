...
const AQUATIC_PLANT_IMAGES = [
  { src: "/plant1.png", alt: "Anubias" },
  { src: "/plant2.png", alt: "Java Fern" },
  { src: "/plant3.png", alt: "Amazon Sword" },
  { src: "/plant4.png", alt: "Dwarf Hairgrass" },
];
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          `${API}/products?categories=69b23691da5543e64646dcb6`,
        );
        ...
        const mapped = data.map((item, index) => ({
          ...
          image: AQUATIC_PLANT_IMAGES[index]?.src || "/plant1.png",
          alt: AQUATIC_PLANT_IMAGES[index]?.alt || item.name,
        }));
        setProducts(mapped);
      } catch (err) {
        console.error("Failed to fetch aquatic plants:", err);
      } finally {
        setLoading(false);
      }
    };
